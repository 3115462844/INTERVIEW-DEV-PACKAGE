...
app.get('/api/buildings', (req, res) => { // 查询所有楼栋
  res.json(db.buildings);
});

// ============ 设备 ============

app.get('/api/devices', (req, res) => { // 查询所有设备，支持根据 buildingId、status、type 过滤
...
app.get('/api/devices/:id', (req, res) => { // 查询单个设备详情，附带该设备的告警信息
...
app.get('/api/alerts', (req, res) => { // 查询所有告警，支持根据 buildingId、level、acknowledged 过滤 
...
app.post('/api/alerts/:id/ack', (req, res) => { // 确认告警
...
const VALID_TRANSITIONS = { // 定义工单状态流转规则
  pending: ['assigned'],
...
app.get('/api/work-orders', (req, res) => { // 查询所有工单，支持根据 status 过滤
...
app.post('/api/work-orders', (req, res) => { // 创建工单，必填 title 和 deviceId，选填 description 和 priority
...
app.patch('/api/work-orders/:id', (req, res) => { // 更新工单状态，首先根据当前工单id查询其在数据库中的状态，然后判断该状态中的status是否可以流转到请求体中的status，如果不可以则返回422错误，如果可以则更新状态并返回更新后的工单信息
...
app.post('/api/chat', (req, res) => {
  ...
  setTimeout(() => {
    ...
    //   console.log(`测试数据[Chat] ${msg.role}: ${msg.content}`);
    // }
    // console.log('------------------------------------');
    if (!lastMsg) {
      return res.json({
...
function detectIntent(text) {
  ...
  // 创建工单意图
  if (/创建工单|新建工单|提交工单|报修/.test(text)) { // 包含工单相关关键词
    // 尝试提取设备ID
    const deviceMatch = text.match(/(elevator|hvac|pump|lighting|fire)_\d+/);
    const titleMatch = text.match(/(?:标题|主题)[：:]\s*(.+?)(?:\s|$|[，,])/);
    return { // 返回工具调用信息，工具名固定为 create_work_order，参数包括 title（如果提取不到则默认为“新维修工单”）、description（原始文本）、deviceId（如果提取不到则为空字符串）、priority（如果文本中包含紧急/严重/高则为 high，否则为 medium）
  ...
  // 查询告警意图
  if (/告警|报警|异常|未确认/.test(text)) { // 包含告警相关关键词
    ...
    return { // 返回工具调用信息，工具名固定为 query_alerts，参数包括 buildingId（如果提取不到则为空字符串）、level（如果提取不到则为空字符串）
      name: 'query_alerts',
  ...
  // 查询设备意图
  if (/设备|电梯|空调|水泵|照明|消防|故障|离线|状态/.test(text)) { // 包含设备类型或状态关键词
    const buildingMatch = text.match(/B(\d)/i);
    const args = {};
    if (buildingMatch) args.buildingId = `B${buildingMatch[1]}`;
    ...
    return { // 返回工具调用信息，工具名固定为 query_devices，参数包括 buildingId（如果提取不到则为空字符串）、status（如果提取不到则为空字符串）、type（如果提取不到则为空字符串）
      name: 'query_devices',
      arguments: JSON.stringify(args),
    };
  }
